...
Script for running single-turn, seed-based user simulation experiments in SAGE.
Generates and evaluates model responses to seed prompts.
"""
...
# Load seed prompts
# Seed prompts are loaded from the input directory for processing.
...
# Simulations are executed for each model and dataset.
...
for model_name in model_list:
    ...
    for dataset_name in os.listdir(input_dir):
        if dataset_name.endswith(".tsv"):# and dataset_name.startswith("van-ch"):
            conversation_file_name = f"data/ConversationsSeeds/{model_name}_{dataset_name}"
            with open(
...
# Results are saved to the specified output files.
